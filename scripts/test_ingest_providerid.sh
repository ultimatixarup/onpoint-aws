...
usage() {
  echo "Usage: $0 <FULL_INGEST_URL> <API_KEY> [EXPECTED_API_KEY_NAME]" >&2
  echo "Example: $0 https://agpzpas961.execute-api.us-east-1.amazonaws.com/v1/ingest/telematics <API_KEY> CEREBRUMX" >&2
}

INGEST_URL="${1:-}"
API_KEY="${2:-}"
EXPECTED_NAME="${3:-CEREBRUMX}"
...
SQS_JSON="$(aws sqs receive-message --region "$REGION" \
  --queue-url "$QUEUE_URL" \
...
SQS_JSON="$SQS_JSON" EXPECTED_NAME="$EXPECTED_NAME" python3 - <<'PY'
import json
import os
import sys

payload = os.environ.get("SQS_JSON", "")
expected = os.environ.get("EXPECTED_NAME", "")
if not payload:
    print("No SQS response payload.")
    sys.exit(1)

data = json.loads(payload)
messages = data.get("Messages") or []
if not messages:
    print("No SQS messages received.")
    sys.exit(1)

attrs = messages[0].get("MessageAttributes") or {}
provider = attrs.get("providerId", {})
value = provider.get("StringValue")
print("MessageBody:")
print(messages[0].get("Body"))
print("MessageAttributes:")
print(json.dumps(attrs, indent=2))

if value != expected:
    print(f"providerId mismatch: expected={expected} actual={value}")
    sys.exit(1)

print(f"providerId matches API key name: {value}")
PY
