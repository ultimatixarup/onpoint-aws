...
@Component({
  selector: 'app-tcosearchfleet',
  templateUrl: './tcosearchfleet.component.html',
  styleUrls: ['./tcosearchfleet.component.scss']
})
export class TcosearchfleetComponent implements OnInit {
  vinData: any
  consumer: any = 'All'
  providerData: any[];
  provderList: any[];
  ...
  ngOnInit(): void {
    ...
    if (sessionStorage.getItem('tcoDetails')) {
      ...
      this.defaultConsumer = tcoBody?.consumer
  ...
  async getAllConsumerss() {
    try {
      ...
      this.consumerFleet = this.consumerFleet.filter(item => !excludedConsumers.has(item.name));
      this.consumerFleet = this.consumerFleet.filter(item => item.name === this.customConsumer);
      this.consumerFleet.sort((a, b) => a.name.localeCompare(b.name));
    } catch (error) {
  ...
  getFLeetList() {
    ...
    this.subscription$.add(
      this.userService.getFleetList(this.customConsumer).subscribe((res: any) => {
        this.fleetList = res;
  ...
  getConsumers() {
    ...
    this.subscription$.add(
      this.userService.getConsumerList().subscribe((res: any) => {
        this.consumerList = res?.data
        this.defaultConsumer = null;
        // this.consumerDetails = this.consumerList.map(item =>{
        //   item.name
        // })
        this.spinner.hide()
      }, err => {
  ...
  resetConsumer(): void {
    this.defaultConsumer = null; // Reset the ng-select value to null
  }
  ...
  showDetails() {
    if (this.fleetIdData) {
      ...
      this.details = true;
      this.subscription$.add(
        this.userService.getFleetProviderData(this.customConsumer, this.fleetIdData).subscribe((res: any) => {
      ...
      this.subscription$.add(
        this.userService.getFleetProviderData(this.customConsumer).subscribe((res: any) => {
          this.providerData = res
          this.totalCount = this.providerData.reduce((acc, provider) => acc + provider.count, 0);
          this.providerData = this.providerData.map(res => {
            return { ...res, 'IsOpenMenu': false }
    ...
    this.subscription$.add(
      this.userService.getVINlist( this.customConsumer,this.fleetIdData).subscribe((res: any) => {
  ...
  selectConsumer(evt) {
    if (evt) {
      ...
      this.spinner.show()
      this.subscription$.add(
        this.userService.getFleetList(evt).subscribe((res: any) => {
          this.fleetList = res
