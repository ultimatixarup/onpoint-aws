...
export class EditTripComponent implements OnInit {
  ...
  get addressControls() {
    ...
    // Function to set the trip date
    setTripDate(date: Date): void {
      const formattedDate = {
        year: date.getFullYear(),
        month: date.getMonth() + 1, // NgbDatepicker months are 1-based
        day: date.getDate(),
      };
      this.tripForm.get('tripDate')?.setValue(formattedDate);
    }
  ...
  fetchTripData(tripDetailId: string): void {
    ...
    this.routeOptimzeService.getTripData(tripDetailId).subscribe(
      data => {
        ...
        this.waypoints = this.tripData.deliveryLocations.map(location => ({
        ...
        const dateValue = new Date(this.tripData.tripStartDateTime);
        this.tripForm.patchValue({
          tripName: this.tripData.tripName,
          tripDate: {
            year: dateValue.getFullYear(),
            month: dateValue.getMonth() + 1,  // NgbDatepicker months are 1-based
            day: dateValue.getDate(),
          },
          //tripDate: dateValue.toISOString().split('T')[0],
          startTime:  dateValue.toTimeString().split(' ')[0].substring(0, 5),
          timeZone: this.tripData.timeZone,
          haltTime: this.tripData.defaultHaltTime
        });

        // Populate the delivery locations if needed
       // const deliveryLocationsFormArray = this.tripForm.get('deliveryLocations') as FormArray;
        this.addresses.clear(); // Clear existing form controls

        this.tripData.deliveryLocations.forEach(location => {
          this.addresses.push(this.fb.group({
            name: [location.name, Validators.required],
            phone: [location.phone],
            email: [location.email, Validators.email],
            street: [location.streetAddress, Validators.required],
            city: [location.city, Validators.required],
            state: [location.state, Validators.required],
            haltTime: [location?.haltTime || ''],
            postalcode: [location.zipcode, Validators.required],
            type: [location.type, Validators.required],
  ...
  async uploadFiles(evt: any) {
    ...
    if (files.length > 0) {
      ...
      console.log(this.fileName, "test_one");
      ...
      if (isAllowedFileType) {
        ...
        try {
          ...
          this.toastr.error('Error reading the file. Please try again.', 'Error'); // Toast for error
        ...
        this.toastr.error('Invalid file type. Please upload a CSV file.', 'Error'); // Toast for error
        // Invalid file type
        this.files = null;
        this.fileName = null;
    ...
    if (1) {
      ...
      const payload = {
        tripDetails: {
          ...
          tripStartDateTime: (new Date(Date.UTC(formValues.tripDate.year, formValues.tripDate.month - 1, formValues.tripDate.day, ...formValues.startTime.split(':')))).toISOString(),
      ...
      this.routeOptimzeService.updateTrip(this.tripId,payload).subscribe(
        response => {
          this.isFormSubmitting = false;
          console.log('Trip created successfully:', response);
          if (isNext) {
            this.router.navigate(['adlp/admin/admindashboard/trip-planning/assign-trip-driver', response.id]);
          } else {
  ...
  setEndAddress(index: number) {
   ...
   createISODateString(formValues) {
    const { year, month, day } = formValues.tripDate;
    ...
    if (
      ...
      throw new RangeError('Invalid date or time values');
    }
  ...
  onDateChange(selectedDate: any) {
    // Convert selected date to desired format (mm/dd/yyyy)
    if (selectedDate) {
      const formattedDate = `${selectedDate.month}/${selectedDate.day}/${selectedDate.year}`;
      this.tripForm.get('tripDate')?.setValue(formattedDate);
    } else {
      this.tripForm.get('tripDate')?.setValue(null); // Reset value if no date selected
    }
  }
  onTripDateChange(event: Event): void {
    const input = event.target as HTMLInputElement;
    if (input.value) {
        const selectedDate = new Date(input.value);
        this.setTripDate(selectedDate);
    }
}
