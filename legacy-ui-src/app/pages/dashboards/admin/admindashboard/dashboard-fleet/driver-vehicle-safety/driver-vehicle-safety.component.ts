...
@Component({
  selector: 'app-driver-vehicle-safety',
  templateUrl: './driver-vehicle-safety.component.html',
  styleUrls: ['./driver-vehicle-safety.component.scss']
})
export class DriverVehicleSafetyComponent implements OnInit {
...
filteredTrips: any[] = [];
  ...
  onTimePeriodChangeData(selectedPeriod: string): void {
    ...
    this.fetchSafetyCarDriverSummary(this.pageNumber, this.pageSize);
  }
...
fetchSafetyCarDriverSummary(page: number, size: number): void {
  // this.loading = true;
  // this.spinner.show();
  ...
  this.tripReportService.getSafetyCarDriverSummary(
    selectedVin,
    startDate,
    endDate,
    page - 1,
    ...
    next: (response) => {
      ...
      this.tripSummaryData = response.vehicleMetrics;
      this.carDriverSaftyTotalData=response
      this.filteredTrips = [...this.tripSummaryData]; // Initialize with all data
      ...
      //   this.timezoneService.getLocalTimeFromUTC(trip.latitude, trip.longitude, trip.refuelTime)
      //     .subscribe(localTime => {
...
convertUTCToTimestamp(utcDateTime: string): number {     // Ensure proper UTC date
 ...
 convertUTCToMonthDay(utcDateTime: string): string {
  ...
  loadVinList(): void {
    const consumer = this.customConsumer || 'Smallboard'; // Use customConsumer from session
...
filterByDriverName() {
  this.filteredTrips = this.tripSummaryData.filter((trip) =>
    trip.driverName?.toLowerCase().includes(this.searchDriverName.toLowerCase())
  );
}
