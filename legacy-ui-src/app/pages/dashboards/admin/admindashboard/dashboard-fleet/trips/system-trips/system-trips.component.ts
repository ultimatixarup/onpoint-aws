...
@Component({
  selector: 'app-system-trips',
  templateUrl: './system-trips.component.html',
  styleUrls: ['./system-trips.component.scss']
})
export class SystemTripsComponent implements OnInit {
...
trips: any;
  ...
  ngOnInit() {
    ...
    this.showRole(); // Get user role and fleet
...
loadTripSummary(page: number, size: number): void {
  ...
  // Priority: selectedVin (vehicle) > selectedDriver (driver's VIN)
  const selectedVinParam = this.selectedVin || this.selectedDriver || '';
  ...
  filterByVin() {
    // Clear driver selection when VIN is selected
    this.selectedDriver = null;
    this.loadTripSummary(this.pageNumber, this.pageSize);
  }
  filterByDriver() {
    // Clear VIN selection when driver is selected
  ...
  loadDriverList(): void {
    const fleetId = this.fleetIdData || ''; // Use selected fleet or empty

    // Get consumer ID from selected fleet if available
    let consumerId: string | undefined = undefined;
    if (this.selectedFleet?.consumer?.id) {
      consumerId = this.selectedFleet.consumer.id.toString();
    }

    this.spinner.show();

    // Call API with consumerId from fleet and fleetId
    this._vehicleService.getAssignDrivers(consumerId, fleetId).subscribe({
      next: (response: any) => {
        // Handle paginated response - extract content array
        const driverData = response?.content || response || [];

        this.driverList = driverData.map((item) => {
...
loadFleetList(): void {
  ...
  this.dashboardservice.getFleetList(this.customConsumer).subscribe({
    next: (res: any) => {
      ...
      // For fleet users, set selectedFleet based on fleetIdData
      if ((this.user === 'role_user_fleet' || this.user === 'role_org_group') && this.fleetIdData) {
...
onFleetChange(event: any): void {
  ...
  // Reset VIN and driver selection when fleet changes
  this.selectedVin = null;
  this.selectedDriver = null;

  // Reload VIN and driver lists for the selected fleet
  this.loadVinList();
  this.loadDriverList();
