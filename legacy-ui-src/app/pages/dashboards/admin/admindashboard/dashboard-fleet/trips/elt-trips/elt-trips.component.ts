...
@Component({
  selector: 'app-elt-trips',
  templateUrl: './elt-trips.component.html',
  styleUrls: ['./elt-trips.component.scss']
})
export class EltTripsComponent implements OnInit {
...
calculateAvgFuelCostBasedOnDateDiff(dateDifference: any): number {
  ...
  loadDriverList(): void {
    ...
    const fleetId = '100224';    // From response.fleet.id
  ...
  onFleetChange(event: any): void {
    ...
    this.fleetIdData = event?.id;
    // Reload data based on selected fleet if needed
    // this.getVehicle(this.page, this.pageCount);
  }
