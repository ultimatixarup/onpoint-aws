...
loadTripSummary(page: number, size: number): void {
  ...
  const selectedVinParam = this.selectedDriver || this.selectedVin || '';// Pass empty string if no VIN is selected
  const selectedFleetId = this.fleetIdData || ''; // Pass fleet ID if selected
  this.tripReportService.getScheduledTripSummary(
    selectedVinParam,
    startDate,
    endDate,
    page - 1,
    ...
    next: (response) => {
      this.tripSummaryData = response.trips.content;
      this.totalPages = response.trips.totalPages;
      this.totalTripCounts = response.trips.totalElements;
      ...
      // console.log(this.tripSummaryData,"eer");
      // this.tripSummaryData.forEach((res) => {
      //   const { id, startTime, endTime, startLat, startLng, endLat, endLng } = res;
      ...
      //       console.log('Received timezone data for start:', timezoneData);
      //       this.tripTimeZones[id] = timezoneData.timeZoneId;

      //       const startTimeStamp = this.convertUTCToTimestamp(startTime);
      //       const trip = tripMap.get(id); // Retrieve trip data by tripId
      ...
      //   }
        ...
        //     console.log('Received timezone data for end:', timezoneData);
    ...
    geometry: any
      ...
      this.tripReportService.getScheduledTripDetails(vin, tripId, startTime, endTime).subscribe(
        (res: any) => {
          ...
          this.wayPoints = res.locationPoints.filter(coords => !isNaN(coords[0]) && !isNaN(coords[1]));
          this.start_end_mark = [];
...
convertUTCToTimestamp(utcDateTime: string): number {     // Ensure proper UTC date
 ...
 convertUTCToMonthDay(utcDateTime: string): string {
  ...
  loadDriverList(): void {
    const consumerId = '877634'; // From response.consumer.id
    const fleetId = this.fleetIdData || '100224'; // Use selected fleet or default
