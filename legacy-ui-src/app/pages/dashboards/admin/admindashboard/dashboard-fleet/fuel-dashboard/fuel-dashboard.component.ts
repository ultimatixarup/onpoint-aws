  ...
  loadFuelData(): void {
   ...
   const selectedVin = this.selectedVin || ''; // Pass empty string if no VIN is selected
    forkJoin({
      ...
      fuelEfficiency: this.dashboardsService.getFuelEfficiency(this.fleetIdData, this.selectedVin, startDate, endDate)
        .pipe(catchError(err => {
      ...
      .subscribe(result => {
        ...
        // Map Fuel Efficiency Data from API
        this.fuelEfficiencyData = res.fuelEfficiency.vehicles ? res.fuelEfficiency.vehicles.map((v: any) => ({
          vehicleName: v.vehicleName,
          vin: v.vinNumber,
          idlingPercentage: v.idlingPercent,
          cityDriving: v.cityDrivingMpg,
          highwayDriving: v.highwayDrivingMpg,
          averageMpg: v.averageMpg,
          idealMpg: v.idealMpg,
          fuelSavingOpportunities: v.fuelSavingOpportunitiesGal,
          costSavingOpportunities: v.costSavingOpportunities,
          fuelEfficiencyScore: v.fuelEfficiencyScore
        })) : [];
      });
  }
  ...
  getReFuelSummeryDetails(vin: string): void {
    ...
    this.tripReportService.getReFuelTripSummary(vin, startDate, endDate).subscribe(
      (res: any) => {
        if (res && res.iceMetrics) {
          ...
          this.refuelDetails.forEach((trip: any, index: number) => {
            // Validate coordinates before calling timezone service
            if (trip.latitude && trip.longitude && !isNaN(trip.latitude) && !isNaN(trip.longitude)) {
              this.timezoneService
                .getLocalTimeFromUTC(trip.latitude, trip.longitude, trip.refuelTime)
                .subscribe({
                  next: (localTime: string) => {
                    // Add or update refuelTime property in the same object
                    this.refuelDetails[index].refuelTime = localTime;
                  },
                  error: (err) => {
                    console.warn('Failed to get timezone for refuel event:', err);
                    // Keep the original UTC time if timezone lookup fails
                    this.refuelDetails[index].refuelTime = trip.refuelTime;
                  }
                });
            } else {
              // Keep original time if coordinates are invalid
              console.warn('Invalid coordinates for refuel event at index', index);
            }
          });
        }
      },
      (error) => {
        ...
        this.toastr.error('Unable to load refuel details. Please try again.');
      }
    );
  }
  loadVinList(): void {
